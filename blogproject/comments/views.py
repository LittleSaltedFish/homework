...
from comments.VerifyCode import VerfiyCode
...
#     c_name = Comment.objects.values('name')
#     if name in c_name:
#         user = Comment.objects.filter(name=name)
#         form = CommentForm(request.POST)
#         if form.is_valid():
#             pass
#
...
def dolog(request):
    ...
    if User.objects.filter(username=uname,password=upassword) is None:
        return redirect(reverse('comments:login'))
    else:
...
def doregister(request):
    ...
    c_name = User.objects.values('username')
    if User.objects.filter(username=username) is None:
        ...
        if code == yzm:
            ...
            return redirect(reverse("comments:register"))
...
def liuyanban(request):
    return render(request,'blog/liuyanban.html')


def doliuyan(request):
    username = request.session.get('username')
    if User.objects.filter(username=username) is not None:
        #print(User.objects.filter(username=username))
        text = request.GET.get('text')
        return render(request,'blog/liuyanban.html',context={'username':username,'text':text})
    else:
        return render(request,'blog/liuyanban.html')
